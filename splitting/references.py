    ...
    for x in range(num_sections):
        ...
        if section["type"] == sectype.ROMX or section["type"] == sectype.ROM0:
            ...
            for x in range(num_patches):
                ...
                if obj_ver < 11:
                    patch["line"] = unpack_file("<I", file)
                patch["offset"], type, rpn_size = unpack_file("<IBI", file)
...
for symbol in globalsyms:
    # Remove any symbols that don't appear in any of our objects
    for symbol2 in dict(globalsyms[symbol]):
        if symbol2 not in globalsyms:
    ...
    # Ignore references from ROM0 to ROM0 if the target symbol is exported
    if symbol in banklesssyms:
        for symbol2 in globalsyms[symbol]:
            if symbol2 in banklesssyms:
                if ":" not in symbol2:
                    globalsyms[symbol][symbol2] = True

finalsyms = {x[::-1].split(":")[0][::-1]: {z[::-1].split(":")[0][::-1]: a for z, a in y.items()} for x, y in globalsyms.items()}

args.out.write(dumps(finalsyms, indent=4))
